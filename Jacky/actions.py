...
def trainingNplotting(fis=None, trn=None, trnLbls=None, chk=None,
    ...
    with tf.Session() as sess:
        ...
        tf.summary.FileWriter(os.path.join('summaries','first'), 
                                            sess.graph)
        ...
        for epoch in range(num_epochs):
            trn_pred, trn_loss, trn_P, trn_C, trn_E = fis.train(sess, trn, trnLbls, bias)
            val_pred, val_loss, val_P, val_C, val_E = fis.infer(sess, chk, chkLbls, bias)
            if epoch % 100 == 0:
                print("Training loss after epoch %i: %f" % (epoch, np.mean(trn_loss)))
            if epoch == num_epochs - 1:
                time_end = time.time()
                print("\n" + "Elapsed time: %f" % (time_end - time_start))
                print("Training loss: %f" % np.mean(trn_loss))
                print("Validation loss: %f" % np.mean(val_loss))
            trn_costs.append(trn_loss)
            val_costs.append(val_loss)
            trn_predList.append(np.mean(np.squeeze(trn_pred) - trnLbls))
            val_predList.append(np.mean(np.squeeze(val_pred) - chkLbls))
        ...
        plt.title("Training - Mean Deviation Between Predicted & Actual Values")
        plt.ylabel("Mean Deviation")
        plt.xlabel("Epochs")
            
        plt.figure(4)
        plt.plot(val_predList)
        plt.title("Validation - Mean Deviation Between Predicted & Actual Values")
        plt.ylabel("Mean Deviation")
        plt.xlabel("Epochs")
